...
ionic_lif_get_abs_stats(const struct ionic_lif *lif, struct rte_eth_stats *stats)
{
	...
	DPDK_SIM_MEM_RD(lif->info, lif->info_pa, lif->info_sz);
...
int
ionic_lif_set_mtu(struct ionic_lif *lif, uint32_t mtu)
{
	struct ionic_admin_ctx ctx = {
		.pending_work = true,
		.cmd.lif_setattr = {
		...
		struct ionic_rx_qcq **rxq_out)
{
	...
	max_frame =
		rte_le_to_cpu_32(lif->adapter->ident.lif.eth.max_frame_size);
	 ...
	 * the info array.
	 */
	max_segs = 1 + (max_frame + RTE_PKTMBUF_HEADROOM - 1) / seg_size;

#ifdef IONIC_MEM_BYPASS
	if (lif->adapter->bypass_range_idx && max_segs >= 2) {
		/*
		 * Update the seg_size to be a single fragment
		 * drawn from the bypass memory region
		 */
		seg_size = max_frame - hdr_seg_size;
		max_segs = 2; /* 1 hdr, 1 bulk payload */
	}
#endif

	IONIC_PRINT(DEBUG, "rxq %u max_frame %u seg_size %u max_segs %u",
		index, max_frame, seg_size, max_segs);
		...
		const uint16_t types, const uint8_t *key, const uint32_t *indir)
{
	...
	ctx.cmd.lif_setattr.rss.addr = rte_cpu_to_le_64(lif->rss_ind_tbl_pa);

	DPDK_SIM_MEM_WR(lif->rss_ind_tbl, lif->rss_ind_tbl_pa,
		sizeof(*lif->rss_ind_tbl) * tbl_sz);
...
ionic_lif_configure(struct ionic_lif *lif)
{
	...
	if (lif->adapter->bypass_range_idx) {
		IONIC_PRINT(DEBUG, "Enabling RX/TX SG for bypass");
