...
static const struct eth_dev_ops ionic_eth_dev_ops = {
	...
	.rx_descriptor_done     = ionic_dev_rx_descriptor_done,
	.rx_descriptor_status   = ionic_dev_rx_descriptor_status,
	...
	.tx_descriptor_status   = ionic_dev_tx_descriptor_status,
...
eth_ionic_dev_init(struct rte_eth_dev *eth_dev, void *init_params)
{
	...
	/* Driver will release all resources in dev_close() */
	...
	struct ionic_adapter *adapter)
{
	if (!strcmp(key, IONIC_CMB_SUPPORT)) {
		...
		return -ENOTSUP;
#endif
	} else if (!strcmp(key, IONIC_MB_BYPASS)) {
#ifdef IONIC_MEM_BYPASS
		char *next;

		adapter->bypass_range_idx = (uint8_t)strtoul(val, &next, 10);
		if (val == next) {
			IONIC_PRINT(ERR,
				"%s '%s' invalid, use range index ('3')",
				IONIC_MB_BYPASS, val);
			return -ERANGE;
		}

		if (adapter->bypass_range_idx == 0)
			IONIC_PRINT(DEBUG, "%s disabled (default)",
				IONIC_MB_BYPASS);
		else
			IONIC_PRINT(NOTICE, "%s enabled on range %u",
				IONIC_MB_BYPASS, adapter->bypass_range_idx);
#else
		RTE_SET_USED(val);
		RTE_SET_USED(adapter);
		IONIC_PRINT(ERR, "%s only supported on embedded configs",
			IONIC_MB_BYPASS);
		return -ENOTSUP;
#endif
	} else {
	...
	uint16_t device_id, uint16_t vendor_id)
{
	...
	for (i = 0; i < bars->num_bars; i++) {
		...
		if (bar->vaddr == NULL) {
			IONIC_PRINT(ERR, "Null bar found, aborting");
			err = -EFAULT;
			goto err_free_adapter;
		}

		adapter->bars.bar[i].vaddr = bar->vaddr;
		adapter->bars.bar[i].bus_addr = bar->bus_addr;
		adapter->bars.bar[i].len = bar->len;
	}
