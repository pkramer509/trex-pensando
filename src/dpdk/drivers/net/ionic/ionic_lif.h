...
//#define IONIC_CODE_PERF_RX
//#define IONIC_CODE_PERF_TX
...
struct ionic_tx_stats {
	...
	uint64_t tx_flush_min;
	uint64_t tx_flush_max;
	uint64_t tx_post_cycles;
	uint64_t tx_post_min;
	uint64_t tx_post_max;
#endif
};
...
struct ionic_rx_stats {
	...
	uint64_t rx_service_cycles;
	uint64_t rx_service_max;
	uint64_t rx_service_min;
	uint64_t rx_calls;
	uint64_t rx_calls_empty;
#endif
};
...
#define IONIC_QCQ_F_BYPASS	BIT(6)
