		...
		void *cb_arg __rte_unused)
{
	...
	DPDK_SIM_DESC_RD(struct ionic_admin_comp,
			cq->base_pa, cq_desc_index, cq_desc);
...
ionic_adminq_post(struct ionic_lif *lif, struct ionic_admin_ctx *ctx)
{
	...
	DPDK_SIM_DESC_WR(struct ionic_admin_cmd,
			q->base_pa, q->head_idx, q_desc);
...
ionic_port_init(struct ionic_adapter *adapter)
{
	...
	DPDK_SIM_DESC_ALLOC(idev->port_info_pa,
		idev->port_info_z->addr, idev->port_info_sz);
