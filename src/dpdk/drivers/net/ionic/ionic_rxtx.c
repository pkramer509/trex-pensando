...
ionic_dev_tx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t tx_queue_id)
{
	...
	if (stats->packets > 0) {
		IONIC_PRINT(DEBUG, "[CODE_PERF] TX queue %u "
			"tx_post_cpp min %ju avg %ju max %ju",
			txq->qcq.q.index,
			stats->tx_post_min,
			stats->tx_post_cycles / stats->packets,
			stats->tx_post_max);
		IONIC_PRINT(DEBUG, "[CODE_PERF] TX queue %u "
			"tx_flush_cpp min %ju avg %ju max %ju",
			txq->qcq.q.index,
			stats->tx_flush_min,
			stats->tx_flush_cycles / stats->packets,
			stats->tx_flush_max);
	}
#endif
		...
		struct rte_mempool *mp)
{
	...
	if (lif->adapter->bypass_range_idx) {
		/* Allocate segments from the bypass pool */
		rxq->mb_seg_pool = ionic_mem_bypass_mpool(socket_id,
					rxq->seg_size, rxq->mb_hdr_pool);
		if (rxq->mb_seg_pool == NULL) {
			IONIC_PRINT(ERR, "Queue %d mbuf pool init failure",
				rx_queue_id);
			return -ENXIO;
		}

		rxq->flags |= IONIC_QCQ_F_BYPASS;
	}
#endif

	DPDK_SIM_MBUF_INIT(nb_desc * rxq->qcq.q.num_segs * 2);
...
#ifdef IONIC_MEM_BYPASS
	if (rxq->qcq.lif->adapter->bypass_range_idx) {
		if (rxq->hdr_seg_size > rxq->frame_size)
			q->num_segs = 1;
		else
			q->num_segs = 2;
	}
...
ionic_dev_rx_queue_stop(struct rte_eth_dev *eth_dev, uint16_t rx_queue_id)
{
	...
	uint64_t rx_calls = stats->rx_calls + stats->rx_calls_empty;
	IONIC_PRINT(DEBUG, "[CODE_PERF] RX queue %u calls %ju empty %ju%%",
		rxq->qcq.q.index, rx_calls,
		rx_calls ? (stats->rx_calls_empty * 100 / rx_calls) : 0);
	if (stats->packets > 0)
		IONIC_PRINT(DEBUG, "[CODE_PERF] RX queue %u "
			"rx_service_cpp min %ju avg %ju max %ju",
			rxq->qcq.q.index,
			stats->rx_service_min,
			stats->rx_service_cycles / stats->packets,
			stats->rx_service_max);
#endif
